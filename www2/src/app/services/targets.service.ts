...
export class TargetsService {
  ...
  async initializeTarget(
    ...
    this.readInboundMessages(target)
      ...
      .subscribe((message: TargetInboundMessage) => {
        console.log(
          `${target.name} --- Message received: ${message.code} as ${
            OutCode[message.code]
          }`
        );
        ...
        if (message.code === OUT_COMPUTER_BUMPER_HIT) {
          ...
          const index =
            message.state?.findIndex((bumper) => bumper.hit === true) ?? -1;
  ...
  connect(target: Target): Observable<boolean> {
    return this.webSerialService.sendMessage(target.connection, [
      IN_COMPUTER_CONNECTED,
    ]);
  }
  ...
  sendMessage(
    ...
    console.log(
      '🚀 ~ file: targets.service.ts:129 ~ TargetsService ~ sendMessage ~ message:',
      message
    );
    switch (message.code) {
  ...
  private createBumpersStateReplaySubject(
    length = 100
  ): ReplaySubject<Bumper[]> {
