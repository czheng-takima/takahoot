...
describe('FirebaseModule', () => {
  ...
  const databaseURL = 'http://testURL.test/database';
  ...
  describe('forRoot', () => {
    it('should provide the firebase admin', async () => {
      const module = await Test.createTestingModule({
        imports: [
          FirebaseModule.forRoot({ googleApplicationCredential, databaseURL }),
        ],
      }).compile();
        ...
        const module = await Test.createTestingModule({
          imports: [
            FirebaseModule.forRootAsync({
              useFactory: () => ({ googleApplicationCredential, databaseURL }),
            }),
          ],
        }).compile();
